...
import com.tencent.qcloud.adapter.ChatGridAdapter;
...
public class ChatInput extends RelativeLayout implements TextWatcher, View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private boolean isSendVisible, isHoldVoiceBtn, isEmoticonReady, isCancelVoice;
    ...
    private GridView panelGridView;

    private int[] imgReses = {/*R.drawable.ic_image,*/
            R.drawable.pic,
            R.drawable.camera,
            R.drawable.position,
            R.drawable.day,
...
//            R.drawable.ic_file
    ...
    private String[] bottomValues = {"照片", "拍摄", /*"小视频", "文件",*/ "位置", "日报", "周报", "月报"};
    ...
    private void initView() {
        ...
        panelGridView = (GridView) findViewById(R.id.more_panel_grid);

        List<InputBean> beanList = new ArrayList<>();
        for (int i = 0; i < imgReses.length; i++) {
            InputBean inputBean = new InputBean();
            inputBean.drawableRes = imgReses[i];
            inputBean.bottomName = bottomValues[i];
            beanList.add(inputBean);
        }
        ChatGridAdapter chatGridAdapter = new ChatGridAdapter(getContext(), beanList);
        panelGridView.setAdapter(chatGridAdapter);

        panelGridView.setOnItemClickListener(this);
...
//        btnFile.setOnClickListener(this);
    ...
    private void cancelVoiceView() {
        chatView.cancelSendVoice(isCancelVoice);
    }


    private void updateVoiceView() {
        if (isCancelVoice) {
            chatView.canceledSendVoice();
...
//        if (adapter == null) {
//            adapter = new EmoticonAdapter(getContext());
//        }
//        emoticonPanel.setAdapter(adapter);
        for (int i = 0; i < 5; ++i) {
            ...
            for (int j = 0; j < 7; ++j) {
                ...
                try {
                    ...
                    image.setLayoutParams(new LinearLayout.LayoutParams(DpUtil.dp2px(getContext(), 24), DpUtil.dp2px(getContext(), 24), 1f));
...
//                }
                break;
            case 3:
                chatView.sendReport(0);
                break;
            case 4:
                chatView.sendReport(1);
                break;
            case 5:
                chatView.sendReport(2);
                break;
        }
    }
