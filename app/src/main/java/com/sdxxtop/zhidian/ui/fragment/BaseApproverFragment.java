...
import com.sdxxtop.zhidian.adapter.ApplyCollectRecyclerAdapter;
...
public abstract class BaseApproverFragment extends BaseFragment implements ApplyHorRecyclerAdapter.HorListener {
    protected int at;
    ...
    private List<SelectBean> selectCollectData;
    private HashSet<Integer> selectPartSet;
    private HashSet<Integer> selectUserSet;
    private HashSet<Integer> selectCollectUserSet;

    @Override
    protected void initVariables() {
        if (getArguments() != null) {
            at = getArguments().getInt("at", -1);
        }
    }
    ...
    protected void initData() {
        ...
        RequestUtils.createRequest().postApplyApprover("approver", params.getData()).enqueue(new RequestCallback<ApproverIndexBean>(new IRequestListener<ApproverIndexBean>() {
            ...
            public void onSuccess(ApproverIndexBean approverIndexBean) {
                ...
                if (data != null) {
                    BaseApproverFragment.this.data = data;
                    initChildData(data);
                } else {
                    showToast("获取审核人数据失败");
                }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            copyPeople(data);
        } else if (requestCode == 101 && resultCode == NoticeReciveRangeActivity.NOTICE_RESULT_OK && data != null) { //收集人
            collectPeople(data);
        }
    }

    protected void collectSkip() {
        Intent intent = new Intent(mContext, NoticeReciveRangeActivity.class);
        startActivityForResult(intent, 101);
    }

    private void collectPeople(Intent data) {
        if (data == null) {
            return;
        }
    ...
    protected String getPartValue() {
        String value = "";
        if (selectPartSet == null) {
            return value;
        }
        for (Integer integer : selectPartSet) {
            value = value + integer + ",";
        }
    ...
    protected String getCollectValue() {
        String value = "";
        if (selectCollectUserSet == null) {
            return value;
        }
        for (Integer integer : selectCollectUserSet) {
            value = value + integer + ",";
        }
        if (value.length() > 0) {
            value = value.substring(0, value.length() - 1);
...
//                intent.putExtra("isPartSelect", NoticeReciveRangeActivity.PART_NOT_SELECTOR);
    ...
    protected ApplyCollectRecyclerAdapter collectAdapter;

    protected void setCollectRecycler(RecyclerView collectRecycler) {
        collectAdapter = new ApplyCollectRecyclerAdapter(new ArrayList<SelectBean>());
        collectRecycler.setAdapter(collectAdapter);
        collectRecycler.setLayoutManager(new LinearLayoutManager(mContext, LinearLayoutManager.HORIZONTAL, false));
    }
