...
import com.sdxxtop.zhidian.widget.SearchResultAdapter;
import com.sdxxtop.zhidian.widget.SegmentedGroup;

import java.util.ArrayList;
import java.util.List;

public class GaoDeMapActivity extends AppCompatActivity implements LocationSource, AMapLocationListener, GeocodeSearch.OnGeocodeSearchListener, PoiSearch.OnPoiSearchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            jindu = getIntent().getDoubleExtra("jindu", -1);
            weidu = getIntent().getDoubleExtra("weidu", -1);
        }
    ...
    private void initView() {
        map_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {  //返回不获取经纬度
//                Intent intent = new Intent();
//                intent.putExtra("ar", area);
...
//                if (weidu != -1 && jindu != -1 && !isFirst) {
//                    isFirst = true;
//                    searchLatlonPoint = new LatLonPoint(weidu, jindu);
//                    aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(weidu, jindu), 16f));
//                } else {
...
//                }
    ...
    public void onActionSearchByKey(View v) {
        searchText.setText("");
    }
...
//        resultData.add(firstItem);
    ...
    AdapterView.OnItemClickListener onItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            area = poiItem.getCityName() + poiItem.getAdName() + poiItem.getSnippet();
            address = poiItem.getTitle().toString();
            Intent intentFirst = new Intent();
            intentFirst.putExtra("ar", area);
            intentFirst.putExtra("ad", address);
            String value = poiItem.getLatLonPoint().getLongitude() + "," + poiItem.getLatLonPoint().getLatitude();
            intentFirst.putExtra("lt", value);
            PreferenceUtils.getInstance(GaoDeMapActivity.this).saveParam(ConstantValue.COMPANY_JIN_WEIDU, value);
            setResult(10087, intentFirst);
            finish();
                /*Intent intentSecond = new Intent();
                intentSecond.putExtra("sa", poiItem.getCityName() + poiItem.getAdName() + poiItem.getSnippet());
                //intent.putExtra("lt", "经纬度");
                setResult(10001, intentSecond);
                //ToastUtil.show(poiItem.getCityName() + poiItem.getAdName() + poiItem.getSnippet());
                finish();*/
