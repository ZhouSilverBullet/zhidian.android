...
import com.sdxxtop.zhidian.adapter.ApplyExamineListAdapter;
 ...
 * 类的描述：
 */
public class ApplyExamineListFragment extends BaseFragment {
    ...
    private String path;
    private int status;
    private ApplyExamineListAdapter mAdapter;
    private HorAdapter horAdapter;
    private RecyclerView horRecyclerView;
    private int showSelect;

    public static ApplyExamineListFragment newInstance(int status, String path, int showSelect /*1表示隐藏，2表示出现*/) {
        Bundle args = new Bundle();
        args.putString("path", path);
        args.putInt("status", status);
        args.putInt("showSelect", showSelect);
        ApplyExamineListFragment fragment = new ApplyExamineListFragment();
...
//            mAdapter.replaceData(applyBeans);
    ...
    private void loadData(final int page, int parentNum, String searchName) {
                ...
                new RequestCallback<>(mContext, new IRequestListener<ApplyListBean>() {
                    ...
                    public void onSuccess(ApplyListBean applyListBean) {
                        ...
                        if (data != null && data.getAll_apply() != null) {
                            List<ApplyListBean.DataBean.ApplyBean> apply = data.getAll_apply();
                            int num = data.getNum();
...
//        List<ApplyListBean.DataBean.ApplyBean> applyBeans = searchIntFilter(apply);
        mAdapter = new ApplyExamineListAdapter(R.layout.item_apply_list_fragment_recycler);
        ...
        mAdapter.setCheckAllListener(new ApplyExamineListAdapter.CheckAllListener() {
            @Override
    ...
    private void loadData(final int page, int parentNum) {
        loadData(page, parentNum, "");
    }
