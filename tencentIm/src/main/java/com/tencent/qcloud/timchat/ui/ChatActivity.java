...
import com.tencent.qcloud.timchat.utils.GetImagePath;
...
public class ChatActivity extends BaseImActivity implements ChatView {
    ...
    public static void navToChat(Context context, String identify, TIMConversationType type) {
        ...
        intent.putExtra("user_add_friend", "userAddFriend");
        context.startActivity(intent);
    }

    //用户群添加的时候使用
    public static void navFriendUserGroupToChat(Context context, String identify, TIMConversationType type) {
        Intent intent = new Intent(context, ChatActivity.class);
        intent.putExtra("identify", identify);
        intent.putExtra("type", type);
        intent.putExtra("user_add_group", "userAddGroup");
        context.startActivity(intent);
    }
...
//        presenter.start();
    ...
    private void initView(Intent intent) {
        ...
        FriendshipInfo.getInstance().setFriendRefreshListener(new FriendshipInfo.FriendRefreshListener() {
            @Override
    ...
    private void refreshTitleLayout() {
        switch (type) {
            ...
            case Group:
                ...
                title.setTitleText(GroupInfo.getInstance().getGroupName(identify));
                if (presenter != null) {
                    presenter.userGroupCount();
                }
                break;

        }

        String messageStringNum = UnreadUtil.getInstance().getUnreadMessageStringNum();
        if (TextUtils.isEmpty(messageStringNum)) {
            title.setLeftText("消息");
        } else {
            title.setLeftText("消息(" + messageStringNum + ")");
        }
    }

    @Override
    public void userGroupCount(int count) {
        switch (type) {
            case C2C:
                title.setTvCount(null);
                break;
            case Group:
                title.setTvCount(count + "");
                break;
        }
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null) {
            String userAddGroup = intent.getStringExtra("user_add_group");
            String userAddFriend = intent.getStringExtra("user_add_friend");
            if (!TextUtils.isEmpty(userAddGroup) || !TextUtils.isEmpty(userAddFriend)) {
                //让他停止观察就好了
                presenter.stop();
                messageList.clear();
...
//        startActivityForResult(intent_album, IMAGE_STORE);
        ...
        rxPermissions.request(Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.CAMERA).subscribe(new Consumer<Boolean>() {
            ...
            public void accept(Boolean aBoolean) throws Exception {
                if (aBoolean) {
                    ...
                    Toast.makeText(ChatActivity.this, "请开启拍照权限", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void takePhoto() {
        ...
        File savePath = new File(getExternalCacheDir() + "/xunxing_photo/");
        if (!savePath.exists()) {
            savePath.mkdirs();
        }
        tempFile = new File(savePath, "img1.png");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            fileUri = FileProvider.getUriForFile(this, "com.sdxxtop.zhidian.FileProvider", tempFile);
        } else {
     ...
     */
    @Override
    public void cancelSendVoice(boolean isCancel) {
        voiceSendingView.showCancel(isCancel);
    }

    @Override
    public void canceledSendVoice() {
        voiceSendingView.release();
        voiceSendingView.setVisibility(View.GONE);
        recorder.stopRecording();
    }
    ...
    public static final int CUSTOMER_TO_LOCATION = 103;
    ...
    public void locationMessage() {
        ...
        startActivityForResult(intent, CUSTOMER_TO_LOCATION);

    }

    public static final int DAY_REPORT = 10;

    @Override
    public void sendReport(int type) {
        switch (type) {
            case 0:
                skipWorkWeeklyActivity(0, 0);
                break;
            case 1:
                skipWorkWeeklyActivity(1, 0);
                break;
            case 2:
                skipWorkWeeklyActivity(2, 0);
                break;
        }
    }

    private void skipWorkWeeklyActivity(int position, int chat_activity) {
        Intent intent = new Intent();
        intent.setClassName("com.sdxxtop.zhidian", "com.sdxxtop.zhidian.ui.activity.WorkWeeklyActivity");
        intent.putExtra("position", position);
        intent.putExtra("chat_activity", 0);
        startActivityForResult(intent, DAY_REPORT);
    }
...
//                showImagePreview(fileUri.getPath());
                Uri newUri = GetImagePath.getImageContentUri(this, tempFile);
                if (newUri == null) {
                    newUri = fileUri;
                }
                Cursor cursor = getContentResolver().query(newUri, new String[]{MediaStore.Images.Media.DATA}, null, null, null);
                if (cursor != null) {
            ...
            //周报
        } else if (requestCode == DAY_REPORT && resultCode == 100 && data != null) {
            String reportJson = data.getStringExtra("report_json");
            int reportType = data.getIntExtra("report_type", -1);
            Log.e(TAG, "reportJson = " + reportJson);
            ...
            elem.setData(reportJson.getBytes());      //自定义 byte[]
            switch (reportType) {
                case 1:
                    elem.setDesc("日报"); //自定义描述信息
                    break;
                case 2:
                    elem.setDesc("周报");
                    break;
        ...
        } else if (requestCode == CUSTOMER_TO_LOCATION && resultCode == 10087 && data != null) {

            double longitude = data.getDoubleExtra("longitude", -1);
            double latitude = data.getDoubleExtra("latitude", -1);

            String address = data.getStringExtra("ad");
            String imageUrlPath = data.getStringExtra("image_url");

            TIMMessage msg = LocationMessage.createMessage(imageUrlPath, latitude, longitude, address);
//            //构造一条消息
//            TIMMessage msg = new TIMMessage();
//
//            //添加位置信息
...
//            elem.setDesc(address);
            ...
            presenter.sendMessage(msg);
        } else if (requestCode == 129 && resultCode == 130) {
            refreshTitleLayout();
        } else if (requestCode == 129 && resultCode == 121) {
            //退群的时候的回调
            finish();
        }
    }
