...
public class VoiceMessage extends Message {
    ...
    public void showMessage(ChatAdapter.ViewHolder viewHolder, Context context) {
        ...
        if (duration > 30) {
            ...
            tvWidth = (int) ((duration / 30.0) * maxWidth);
        }

        LinearLayout.LayoutParams tvLp = new LinearLayout.LayoutParams(
                tvWidth < ViewUtil.dp2px(context, 30) ? ViewGroup.LayoutParams.WRAP_CONTENT : tvWidth
                , ViewGroup.LayoutParams.WRAP_CONTENT);
        int height = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 18, displayMetrics);
        int width = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 14, displayMetrics);
